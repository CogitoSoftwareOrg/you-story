...
export class ScenePolicyEngine {
	get(enhance: EnhanceOutput): ScenePolicy {
		...
		if (userEmotion === UserEmotion.Excited || userEmotion === UserEmotion.Good) {
			...
			if (
				detailLevel === DetailLevel.Medium &&
				interactionIntent !== SceneIntent.HighTensionAction
			) {
				detailLevel = DetailLevel.High;
			}
		}
...
export const scenePolicyEngine = new ScenePolicyEngine();
