...
ONLY Available characters to use:
{characters}
- World (give general environment context)
...
class CinemaStoryteller implements Storyteller {
	streamStory(
		...
		const availableCharacters = chars.filter((char) => char.id !== chat.data.povCharacter);
		const povCharacter = chars.find((char) => char.id === chat.data.povCharacter);
	...
	private buildMessages(
		...
		userMsg: MessagesResponse,
		povCharacter?: CharactersResponse
	): OpenAIMessage[] {
		...
		console.log(chatMessages.map((m) => m.content).join('\n'));
		...
		messages.push({
			role: 'user',
			content: `User (${povCharacter?.name ?? 'World'}) message: ${userMsg.content}`
		});
